...
def link_to_list(link):
    ...
    List = []
    start = link
    while start != Link.empty:
        List.append(start.first)
        start = start.rest
    return List
...
def in_order_traversal(t):
    ...
    if t.is_leaf():
        ...
        for i in list(in_order_traversal(t.branches[0])):
            yield i
        yield t.label
        for j in list(in_order_traversal(t.branches[1])):
            yield j
