...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        if self.string in env:
            ...
            raise NameError('The name does not exist in this environment.')
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        operand_values = [val.eval(env) for val in self.operands]
        return function.apply(operand_values)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        par_value = zip(self.parameters, arguments)
        for i in par_value:
            env_copy[i[0]] = i[1]
